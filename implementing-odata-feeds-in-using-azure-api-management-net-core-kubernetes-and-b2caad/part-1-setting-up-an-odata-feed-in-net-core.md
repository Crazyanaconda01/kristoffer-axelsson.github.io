...
The single most important line of code below is probably the [EnableQuery] as it makes the feed queryable, which was the business need, based on the 
...
### Meta data:
The above are the options for the Swagger API documentation generation. Of particular note here is the options.DocumentFilter<AddOdataMetadataEndpointsFilter>(); line. By default Swagger 
